...
namespace Ada.Helpers
{
    ...
    {
        public static IServiceCollection AddServicesByNamespaceOfClassWithBase<Class, Base>(this IServiceCollection services)
        {
            IEnumerable<Type> commands = typeof(Class)
                .Assembly
                .GetExportedTypes()
                .Where(x => x.Namespace == typeof(Class).Namespace && x.BaseType == typeof(Base));
        ...
        public static void SetDefaultResponseLocation(this ITurnContext<IMessageActivity> turnContext)
        {
            if (turnContext.Activity.Value is EventType slackEvent)
            {
                if (slackEvent.AdditionalProperties["channel_type"].ToString() == "channel")
                {
                    turnContext.Activity.Conversation.Properties["thread_ts"] = turnContext.Activity.Id;
                }
            }
        }

        public static string NextWordAfter(this string toBeSearched, string searchTerm)
        {
            int index = searchTerm.IndexOf(toBeSearched, StringComparison.CurrentCultureIgnoreCase);

            if (index == -1)
            {
                throw new Exception($"Failed to find next word. `{searchTerm}` was not found in `{toBeSearched}`.");
            }

            return searchTerm[(index + toBeSearched.Length)..].Split(' ')[0];
        }

        public static async Task<User> GetUserById(this SlackTaskClient slackClient, string id)
        {
            var users = await slackClient.GetUserListAsync();
            return users.members.First(x => x.id == id);
        }

        /// <summary>
        /// A helper class that creates and populate <see cref="RecognizerResult"/> from a <see cref="AnalyzeConversationResult"/> instance.
        /// </summary>
        public static class RecognizerResultBuilder
        {
